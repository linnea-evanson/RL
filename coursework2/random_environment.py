...
class Environment:
    ...
    def draw_greedy_pol(self, greedy_states):
        # Create the background / obstacle
        window_top_left = (0, 0)
        window_bottom_right = (self.magnification * 1, self.magnification * 1)
        cv2.rectangle(self.image, window_top_left, window_bottom_right, (50, 50, 50), thickness=cv2.FILLED)
        ...
        for block in self.free_blocks:
            ...
            bottom_right = (int(self.magnification * block[1][0]), int(self.magnification * (1 - block[1][1])))
            cv2.rectangle(self.image, top_left, bottom_right, (246, 238, 229), thickness=cv2.FILLED)
        # Draw the agent (initial state)
        agent_state = greedy_states[0]
        agent_centre = (int(agent_state[0] * self.magnification), int((1 - agent_state[1]) * self.magnification))
        agent_radius = int(0.01 * self.magnification)
        agent_colour = (50, 50, 200)
        cv2.circle(self.image, agent_centre, agent_radius, agent_colour, cv2.FILLED)
        ...
        #Draw all steps the agent takes with greedy policy
        line_colour = (0,255,0) #lime green
        line_thickness = agent_radius
        for i in range(len(greedy_states) - 1):
            point1 = greedy_states[i]
            point2 = greedy_states[i+1]

            point1_centre = (int(point1[0] * self.magnification), int((1 - point1[1]) * self.magnification))
            point2_centre = (int(point2[0] * self.magnification), int((1 - point2[1]) * self.magnification))

            cv2.line(self.image, point1_centre, point2_centre, line_colour, line_thickness)

        title = "Greedy policy"
        png = "Greedy_pol_random_env" + str( int(np.random.uniform(0,1000000))) +   ".png"   #create random name for the image to be saved
        # Show the image
        cv2.imshow(title, self.image)
        cv2.imwrite(png, self.image)
        # This line is necessary to give time for the image to be rendered on the screen
        cv2.waitKey(1)