...
class QValueVisualiser:
    ...
    def __init__(self, environment, magnification=500, starting_pos = (0.3500, 0.1500), title = "Qvalues.png"):
        ...
        self.starting_pos = (int(starting_pos[0] * self.magnification), int((1 - starting_pos[1]) * self.magnification))
        self.title = title
    ...
    def draw_q_values(self, q_values):
        ...
        #cv2.circle(self.q_values_image, self.starting_pos, int(0.02 * self.magnification), (100, 199, 246), cv2.FILLED)
        # Draw the grid cells
        self._draw_grid_cells()
        # Show the image
        cv2.imwrite(self.title, self.q_values_image)
