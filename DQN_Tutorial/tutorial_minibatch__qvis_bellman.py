...
class DQN:
    ...
    def _calculate_loss(self, transition_mini_batch):
        ...
        prediction2 = self.q_network(s_prime)  # prediction is the Q value for the state and each of the 4 actions.
...
if __name__ == "__main__":
    ...
    visualiser = QValueVisualiser(environment=environment, magnification=500, title = "Qvalues_bellman.png")   #not sure if this is working correctly or not???
    # Draw the image
    ...
    for i in range(EPISODE_LENGTH - 1):
        ...
        states.append(state)
    print(states)
    environment.draw_greedy_pol(states, title = "Greedy Policy using Bellman Equation", png = "GreedyPol_bellman.png")
    ...
    fig, ax = plt.subplots()
    ax.set(xlabel='Iteration', ylabel='Loss', title='Loss Curve using Bellman Equation')

    # Plot and save the loss vs iterations graph
    ax.plot(iterations, losses, color='blue')
    plt.yscale('log')
    plt.show()
    fig.savefig("loss_bellman.png")